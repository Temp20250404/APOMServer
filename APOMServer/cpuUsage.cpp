...
CCpuUsage::CCpuUsage(HANDLE hProcess)
{
	...
	if (hProcess == INVALID_HANDLE_VALUE)
	{
		...
		// OpenProcess를 통해 실제 프로세스 핸들을 얻기
...
void CCpuUsage::UpdateCpuTime()
{
	...
	// 프로세스에 아직 값이 들어오지 않았거나 프로세스가 멈춰서 측정이 안된 경우.
	if (Total == 0)
	{
	...
	else
	{
		...
		// 여기서 측정할 것은 진짜 프로세스가 유효하게 돌아가고 있던 시간을 측정하는 것이다.
		_fProcessorTotal = (float)((double)(Total - IdleDiff) / Total * 100.0f);	// %로 변환.
	...
	// 전체 시간을 논리 프로세서 갯수로 나누는 이유는 GetProcessTimes 함수가 해당 프로세스에서 작동하고 있는 모든 스레드가 소모한 시간을 의미하기 때문이다.
	// 프로세스의 전체 스레드 갯수가 얼마든 그 스레드는 프로세서가 작동시켜주는 것이고, 프로세서가 제공하는 논리프로세서 중 하나에서 스레드가 도는 시간을 측정한다.
	// 그래서 논리프로세서 갯수로 시간을 나누면 해당 프로세스가 프로세서 위에서 소모한 사용률을 알 수 있다.
